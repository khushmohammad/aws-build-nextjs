...
function Commentapi() {
    ...
    const getCommentList = async () => {
        const res = await getCommentbyPostId("63e3a0e01b2f4232326a3c34", 1, 50, 0, "")
...
const CommentReplyViewOne = ({ parentId, commmetLevel }) => {
    ...
    const getCommentReplyList = async () => {
        const res = await getCommentbyPostId("63e3a0e01b2f4232326a3c34", 1, 50, updatedLevel, parentId)
        const commentrepArr = await res?.comments
...
const CommentList = ({ comment }) => {
    ...
    const FormSubmit = async (e) => {
        e.preventDefault()
        const res = await postCommentByPostId("63e3a0e01b2f4232326a3c34", newReplyText, 0, comment._id)
        const result = await res?.data?.body
                            ...
                            <>
                                {newReply && newReply.map((data, index) => {
                                    return (
                                        <>
                                            {data &&
                                                <CommentReplyViewOne parentId={comment._id} commmetLevel={data.level - 1} />
                                            }
