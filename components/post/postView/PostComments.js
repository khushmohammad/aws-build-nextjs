  ...
  const onSubmit = async (data) => {
    ...
    if (result.status == 200) {
      ...
      commentlist && commentlist.length > 0
        ? setCommentlist([...commentlist, dataWithUserDetails[0]])
        : setCommentlist([dataWithUserDetails[0]]);
      reset();
    } else {
          ...
          (commentlist || []).map((comment, i) => {
            return (
              <React.Fragment key={i}>
                {commentlist && (
                  <CommentLiComponent comment={comment} postId={postId} />
                )}
              </React.Fragment>
            );
          })}
...
function CommentLiComponent({ comment, postId, level, parentId }) {
  ...
  const onFormSubmit = async (e) => {
    ...
    const res = await postCommentByPostId(
      postId,
      commentInputText,
      updatedLevel,
      comment._id
  ...
  const NestedComments = ({ parentId, level, commentReply }) => {
    ...
    return (
      <React.Fragment>
        {commentReplyNested &&
          (commentReplyNested || []).map((replydata, i) => {
            return (
              <React.Fragment key={i}>
                {replydata && (
                  <CommentLiComponent
      ...
      <li className={`mb-2 `}>
        <div className="d-flex">
          ...
          <div className="comment-data-block ms-3 flex-grow-1">
            <h6>
              {comment?.userDetails?.userInfo?.firstName || ""}{" "}
            ...
            <div className="d-flex flex-wrap align-items-center comment-activity">
              ...
              <span className="text-primary">
                {comment?.createdAt && getPostTime(comment.createdAt)}{" "}
              </span>
        ...
        <>
          {comment?.repliesCounts > 0 && (
            <p
              ...
              className="text-primary "
            >
              {viewReply ? "hide" : "view"} reply{" "}
            </p>
          )}

          {viewReply && (
            <ul
              className={`post-comments list-inline  ${
                comment?.level < 2 ? "ms-4" : ""
              }`}
            >
              <NestedComments
                parentId={comment?._id}
