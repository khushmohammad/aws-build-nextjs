...
import { AcceptHelp } from "../../services/chat.socket";
...
function NotificatonCard({ notification, page }) {
  ...
  //   accept help method
  const acceptHelpClick = async (routeId) => {
    try {
      const res = await AcceptHelp(routeId);
      console.log(res);
    } catch (err) {
      console.log(err);
    }
          ...
          <ul className="notification-list m-0 p-0">
            <li className="d-flex align-items-center justify-content-between">
              <div className="user-img img-fluid">
                <Image
                  src={
                    notification?.userDetails?.profilePictureInfo?.file
                      ?.location || user1
                  }
                  alt="profile-bg"
                  height={100}
                ...
                <div className="d-flex justify-content-between">
                  <div className="ms-3">
                    ...
                    <h6>
                      ...
                      <strong>
                        {" "}
                        {notification?.userDetails?.userInfo?.firstName ||
                          ""}{" "}
                        {notification?.userDetails?.userInfo?.lastName || ""}{" "}
                      </strong>{" "}
                  ...
                  {page != "headerNotification" ? (
                    <div className="d-flex align-items-center">
                      {notification?.message == "COMMUNITY_NEED_HELP" ? (
                        <button
                          className="btn btn-primary me-5"
                          onClick={(e) =>
                            acceptHelpClick(notification?.routeId)
                          }
                        >
                          Accept
                        </button>
